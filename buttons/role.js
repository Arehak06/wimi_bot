    ...
    run: async (bot, interaction, parameters) => {
        ...
        if (!interaction.guild)
            ...
            if (member.roles.cache.has(role.id)){
                ...
                return interaction.reply({ content: `Удалена роль ${role.name}`, ephemeral: true})
            }
            // member doesn't have the role => add role
            else {
                await member.roles.add(role.id)
                return interaction.reply({ content: `Добавлена роль ${role.name} для вас!`, ephemeral: true})
            }
